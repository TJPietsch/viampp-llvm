...
//===----------------------------------------------------------------------===//
// VIAMPP ABI Implementation.
//===----------------------------------------------------------------------===//

namespace {
class VIAMPPABIInfo: public DefaultABIInfo {
public:
  VIAMPPABIInfo(CodeGen::CodeGenTypes &CGT) : DefaultABIInfo(CGT) {}
};

class VIAMPPTargetCodeGenInfo : public TargetCodeGenInfo {
public:
  VIAMPPTargetCodeGenInfo(CodeGenTypes &CGT)
      : TargetCodeGenInfo(std::make_unique<DefaultABIInfo>(CGT)) {}
...
/* TODO check with backend later
    auto *Fn = cast<llvm::Function>(GV);

    if (FD->getAttr<VIAMPPInterruptAttr>())
      Fn->addFnAttr("interrupt");

    if (FD->getAttr<VIAMPPSignalAttr>())
      Fn->addFnAttr("signal");
*/
  }
};
...
const TargetCodeGenInfo &CodeGenModule::getTargetCodeGenInfo() {
  ...
  case llvm::Triple::viampp:
    return SetCGInfo(new VIAMPPTargetCodeGenInfo(Types));
  }
}
